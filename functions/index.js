...
app.use('/services', (req, res, next) => {
  connectDb().then(() => {
    createServicesRouter(services, authMiddleware, requireRole)(req, res, next);
  }).catch(next);
});
app.use('/skills', (req, res, next) => {
  connectDb().then(() => {
    createSkillsRouter(skills, authMiddleware, requireRole)(req, res, next);
  }).catch(next);
});
app.use('/clients', (req, res, next) => {
  connectDb().then(() => {
    createClientsRouter(clients, authMiddleware, requireRole)(req, res, next);
  }).catch(next);
});
