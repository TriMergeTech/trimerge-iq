...
function SuccessCheckIcon() {
  return (
    <svg className="h-14 w-14" viewBox="0 0 64 64" fill="none" aria-hidden="true">
      <circle cx="32" cy="32" r="30" fill="#EAF1FF" stroke="#5E94EA" strokeWidth="2" />
      <path d="M20.5 33.5l8 8 15-15" stroke="#3E74D3" strokeWidth="4" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  );
}
...
export default function VerifyPage() {
  ...
  function validate(values) {
    ...
    if (!email.trim()) nextErrors.email = "Missing signup email. Please sign up again.";
  ...
  async function handleSubmit(event) {
    ...
    try {
      const res = await fetch("https://trimerge-iq.onrender.com/auth/verify", {
        method: "POST",
        headers: {
      ...
      setStatusMessage(payload?.message || "Verification failed. Please check the code and try again.");
      setStatusType("error");
    } catch (error) {
  ...
  async function handleResendCode() {
    ...
    try {
      ...
      for (const endpoint of endpoints) {
        const currentResponse = await fetch(`https://trimerge-iq.onrender.com${endpoint}`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
      ...
      if (!response) {
        setStatusMessage("Unable to resend code right now. Please try again later.");
        setStatusType("error");
        return;
      }
      ...
      if (response.ok) {
        setStatusMessage(payload?.message || "A new verification code has been sent to your email.");
        setStatusType("success");
        return;
      }

      setStatusMessage(payload?.message || "Unable to resend code. Please try again.");
      setStatusType("error");
    } catch (error) {
  ...
  return (
    <>
      <Head>
        <title>Verify Account | TriMerge IQ</title>
      </Head>
      <main className="min-h-screen bg-[#ECEFF6] px-4 py-10 sm:px-6 sm:py-14">
        ...
        <section className="rounded-3xl bg-white/90 p-6 shadow-[0_12px_35px_rgba(26,61,130,0.12)] ring-1 ring-white/70 sm:p-8">
          <h2 className="mb-2 text-center text-2xl font-semibold text-[#334E8A]">Verify Account</h2>
          <p className="mb-6 text-center text-sm text-slate-600">Check your email and enter the verification code.</p>

          {isVerified ? (
            <div className="space-y-6">
              <div className="flex flex-col items-center text-center">
                <SuccessCheckIcon />
                <h3 className="mt-4 text-xl font-semibold text-[#334E8A]">Account verified successfully</h3>
                <p className="mt-2 text-sm text-slate-600">Your account is now active. Redirecting to login in {redirectSeconds}s...</p>
                <button
                  type="button"
            ...
            <form onSubmit={handleSubmit} className="space-y-4" noValidate>
              <div className="text-center">
                ...
                <p className="mt-1 text-base font-semibold text-[#334E8A] break-all">{email || "your email address"}</p>
              </div>
