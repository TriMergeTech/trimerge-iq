"use client";

import { Check, Copy, Link2, Loader2, X } from "lucide-react";

interface ShareDialogProps {
  copied: boolean;
  description: string;
  isLoading?: boolean;
  onClose: () => void;
  onCopy: () => void;
  title: string;
  value: string;
}

export default function ShareDialog({
  copied,
  description,
  isLoading = false,
  onClose,
  onCopy,
  title,
  value,
}: ShareDialogProps) {
  return (
    <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/55 px-4 py-4 backdrop-blur-sm">
      <div className="w-full max-w-[520px] overflow-hidden rounded-[24px] border border-white/[0.08] bg-[linear-gradient(180deg,rgba(20,27,39,0.98),rgba(10,15,23,0.98))] shadow-[inset_0_1px_0_rgba(255,255,255,0.055),0_30px_90px_rgba(0,0,0,0.52)] animate-fade-rise">
        <div className="flex items-start justify-between gap-4 border-b border-white/[0.07] px-6 py-5">
          <div className="flex min-w-0 items-start gap-3">
            <span className="mt-0.5 flex h-10 w-10 shrink-0 items-center justify-center rounded-full border border-[#d4af37]/22 bg-[#d4af37]/10 text-[#f4df91]">
              <Link2 className="h-4 w-4" />
            </span>
            <div className="min-w-0">
              <p className="text-[11px] font-semibold uppercase tracking-[0.24em] text-[#f0d98a]/58">Share</p>
              <h2 className="mt-1 truncate text-[19px] font-medium tracking-tight text-white">{title}</h2>
              <p className="mt-1 text-sm leading-6 text-white/52">{description}</p>
            </div>
          </div>

          <button
            type="button"
            aria-label="Close share dialog"
            onClick={onClose}
            className="interactive-button flex h-10 w-10 shrink-0 items-center justify-center rounded-full border border-white/[0.07] bg-white/[0.035] text-white/78 hover:bg-white/[0.07]"
          >
            <X className="h-4 w-4" />
          </button>
        </div>

        <div className="px-6 py-5">
          <div className="rounded-[18px] border border-white/[0.08] bg-black/[0.18] p-3 shadow-[inset_0_1px_0_rgba(255,255,255,0.025)]">
            {isLoading ? (
              <div className="flex items-center gap-3 px-2 py-4 text-sm text-white/58">
                <Loader2 className="h-4 w-4 animate-spin text-[#d4af37]" />
                Creating share link...
              </div>
            ) : (
              <input
                readOnly
                value={value}
                className="w-full bg-transparent px-2 py-3 text-sm text-white outline-none selection:bg-[#d4af37]/24"
                onFocus={(event) => event.target.select()}
              />
            )}
          </div>

          <div className="mt-5 flex justify-end gap-3">
            <button
              type="button"
              onClick={onClose}
              className="interactive-button rounded-full border border-white/[0.08] px-5 py-2.5 text-sm font-semibold text-white/68 hover:bg-white/[0.05] hover:text-white"
            >
              Close
            </button>
            <button
              type="button"
              onClick={onCopy}
              disabled={isLoading || !value}
              className="interactive-button flex items-center gap-2 rounded-full border border-[#f1d46a]/24 bg-[linear-gradient(180deg,#e2be4b,#c79d22)] px-5 py-2.5 text-sm font-semibold text-[#111214] shadow-[inset_0_1px_0_rgba(255,255,255,0.24),0_12px_28px_rgba(212,175,55,0.22)] hover:bg-[linear-gradient(180deg,#edcb62,#d4af37)] disabled:cursor-not-allowed disabled:border-white/[0.06] disabled:bg-none disabled:bg-white/[0.07] disabled:text-white/32 disabled:shadow-none"
            >
              {copied ? <Check className="h-4 w-4" /> : <Copy className="h-4 w-4" />}
              {copied ? "Copied" : "Copy"}
            </button>
          </div>
        </div>
      </div>
    </div>
  );
}

