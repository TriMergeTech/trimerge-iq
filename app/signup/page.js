...
function UserIcon() {
  return (
    <svg
      ...
      aria-hidden="true"
    >
      <path d="M10 2.5a4.1 4.1 0 0 0-4.1 4.1A4.1 4.1 0 0 0 10 10.8a4.1 4.1 0 0 0 4.1-4.2A4.1 4.1 0 0 0 10 2.5ZM3.2 16.7c0-2.7 3-4.6 6.8-4.6s6.8 1.9 6.8 4.6V18H3.2v-1.3Z" />
    </svg>
  );
}
...
export default function SignupPage() {
  ...
  function validate(values) {
    ...
    if (values.password && !passwordRule.test(values.password)) {
      nextErrors.password =
        "Use 8 or more characters with at least one uppercase letter, one number, and one special character.";
    }
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      let res = await post_request(`$PROFILE/add_profile`, {
        details,
        profile,
      });

      if (res?.ok) {
        router.push(`/verify?email=${formValues.email}&profile=${profile}`);
        return;
      }
        ...
        <section className="rounded-3xl bg-white/90 p-6 shadow-[0_12px_35px_rgba(26,61,130,0.12)] ring-1 ring-white/70 sm:p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-4" noValidate>
            ...
            <p className="-mt-2 text-xs text-slate-500">
              Use 8 or more characters with at least one uppercase letter, one
          ...
          <div className="mt-7 border-t border-slate-200 pt-6 text-center">
            ...
            <Link
              href="/login"
              className="mt-2 inline-block text-base font-semibold text-[#4A82DE] transition-colors hover:text-[#396FC5]"
            >
              Log in
            </Link>
          </div>
